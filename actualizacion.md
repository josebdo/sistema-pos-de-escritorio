...
Compara el "Último commit verificado" con el estado actual de git (git log, git status, git diff si hace falta). Si hay commits posteriores no reflejados en el estado, adviértelo antes de continuar.
Inspecciona la estructura actual del proyecto.
Revisa el código relacionado con la fase actual.
Revisa los tests existentes.
Ejecuta build y tests usando los comandos de docs/COMMANDS.md para verificar el estado real, no el declarado.
Determina exactamente dónde quedó el proyecto.
NO asumas que una fase está terminada solamente porque PROJECT_STATE.md diga que lo está — la palabra final la tiene el código compilando y los tests pasando.
Si existe una diferencia entre PROJECT_STATE.md y el código real, informa la diferencia explícitamente y corrígela en el archivo antes de continuar.
...
Ejecuta el build (comando de docs/COMMANDS.md).
Ejecuta los tests correspondientes.
Comprueba errores.
Actualiza PROJECT_STATE.md, incluyendo el hash del último commit y el resultado real de build/tests.
Indica qué archivos fueron modificados.
Indica qué quedó pendiente.
Indica exactamente cuál debe ser el siguiente paso (concreto, accionable).
Poda PROJECT_STATE.md: elimina de "Cambios recientes" todo lo que no sea el último cambio; mueve detalles históricos a docs/CHANGELOG.md si aún no están ahí.
...
Documenta el bloqueo en detalle en "Problemas conocidos" (qué se intentó, por qué falló cada intento).
No sigas intentando el mismo enfoque sin variación.
Pregúntame explícitamente cómo proceder: cambiar de enfoque, replantear el alcance de la fase, o aceptarlo como deuda técnica documentada.
...
Venta de celulares — cada unidad se rastrea individualmente por IMEI/número de serie, no por cantidad genérica. Cada celular vendido tiene su propia garantía.
Venta de accesorios de celulares — covers, auriculares, proyectores de pantalla, etc. Estos SÍ se manejan como inventario normal por cantidad (como cualquier producto de la fase de Productos), sin necesidad de número de serie individual.
Reparación de celulares — un servicio, no un producto con stock. Se recibe el equipo del cliente, se diagnostica/repara, y se cobra al entregarlo. Flujo definido como simple: no se requiere un seguimiento de múltiples estados intermedios (diagnosticando, esperando pieza, etc.) — basta con registrar el equipo, el problema, y marcarlo como listo para cobrar y entregar.
Papelería — venta de artículos de oficina/escolares. Se maneja igual que los accesorios: inventario normal por cantidad, sin particularidades adicionales. Entra dentro del mismo catálogo de Productos, simplemente con sus propias categorías.
...
Qué motor de base de datos se usa en cada modo.
Cómo se migran los datos existentes (productos, ventas, turnos, usuarios) de un negocio que empezó en caja única y pasa a multi-caja, sin perder su historial.
Opciones a evaluar conmigo antes de iniciar esta fase:

Motor cliente-servidor en red local (ej. SQL Server Express, MySQL, PostgreSQL) corriendo en una PC "servidor" dentro del negocio, con las demás cajas conectándose a esa. Coherente con DEC-002 (EF Core separa la app del motor de base de datos, así que cambiar de proveedor es factible sin rehacer la lógica de negocio). Al activar multi-caja en un negocio que ya tenía caja única en SQLite, se migran los datos existentes al nuevo motor (un proceso de exportación/importación que hay que construir como parte de esta fase, no algo manual e improvisado). Esta es la recomendación inicial.
Mantener SQLite pero con un servicio/API intermedio corriendo en la PC servidor, y las demás cajas hablan con esa API en vez de tocar el archivo directamente. Más trabajo de desarrollo, pero evita instalar un motor de base de datos adicional.
Cada caja con su propia base de datos local y sincronización periódica. Más complejo y con riesgo real de conflictos (ej. el mismo producto vendido dos veces si el inventario no sincroniza a tiempo) — no recomendada como primera opción.
...
Orden sugerido
Fase: Sistema de roles y permisos — primero, ya que login y permisos son la base sobre la que se construyen las demás pantallas.
Fase: Clientes — necesaria temprano porque tanto ventas como reparaciones dependen de tener un cliente asociado.
Fase: Apertura y cierre de caja (turno) — depende de tener usuarios autenticados.
Fase: Productos, categorías y SKU — base del inventario, incluyendo el modelo mixto (celulares con IMEI vs. accesorios/papelería por cantidad).
Fase: Proveedores
Fase: Compras a proveedor (actualiza inventario)
Fase: Alertas de stock mínimo
Fase: Reparaciones de celulares — depende de Clientes y Roles; no depende de Productos porque es un servicio, no inventario.
Fase: Gastos e ingresos del negocio
Fase: Código de barras — lectura
Fase: Código de barras — generación
Fase: Métodos de pago — usados tanto por ventas como por reparaciones al momento de cobrar.
Fase: Modo caja única / multi-caja — al final, porque es la decisión arquitectónica de mayor impacto y afecta a todas las fases anteriores; conviene construir y probar bien el sistema en modo caja única primero, y luego agregar la capacidad de activar multi-caja.
...
NO reinicies el proyecto. NO vuelvas a crear archivos existentes sin necesidad. NO cambies la arquitectura sin justificación registrada en docs/DECISIONS.md. NO repitas fases ya completadas.
