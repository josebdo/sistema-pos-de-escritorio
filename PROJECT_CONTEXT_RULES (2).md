...
Alcance mínimo:
- Registrar una orden de reparación (permiso `Reparaciones.Gestionar` — rol Técnico, Admin, Super Admin): cliente (FK a la fase de Clientes — obligatorio, ya que confirmaste que siempre se registra), marca y modelo del equipo, IMEI del equipo (texto libre — es el equipo del cliente, no inventario de la tienda, así que no se valida contra `UnidadProducto`), descripción del problema reportado, precio acordado (puede ajustarse al momento de entregar si el precio final difiere del estimado inicial), fecha de recepción.
- Estado de la orden, simplificado a lo esencial para poder cobrar y entregar: **En reparación** → **Lista para entrega** → **Entregada**. No se contemplan sub-estados (diagnosticando, esperando pieza, etc.) en esta fase — si más adelante quieres agregarlos, es una extensión simple de este mismo modelo. Solo quien tiene `Reparaciones.Gestionar` (Técnico, Admin, Super Admin) puede mover una orden de "En reparación" a "Lista para entrega".
- Al marcar una orden como "Entregada" (permiso `Reparaciones.Cobrar` — rol Cajero, Admin, Super Admin), se registra el cobro (usando los métodos de pago ya definidos: efectivo, transferencia, tarjeta) y la fecha de entrega. El Cajero ve las órdenes en estado "Lista para entrega" pero no puede crear ni editar el diagnóstico/detalle técnico de una orden — esa parte es exclusiva del Técnico.
...
- **Cajero (rol por defecto)**: opera la caja y las ventas. Permisos: registrar ventas, cobrar (efectivo/transferencia/tarjeta), abrir y cerrar turno, ver clientes (buscar/crear rápido), **ver y cobrar reparaciones que ya están "Lista para entrega"** (marcarlas como "Entregada" y registrar el pago) — pero NO puede crear ni editar una orden de reparación en sí (eso es del Técnico). Sin acceso a configuración, reportes financieros completos, ni gestión de usuarios.
- **Técnico (rol por defecto, distinto del Cajero)**: gestiona el servicio de reparación. Permisos: crear y editar órdenes de reparación (marca, modelo, IMEI, problema, precio, cambiar estado a "En reparación" / "Lista para entrega"), ver clientes (para asociarlos a una reparación), ver productos en modo lectura (para consultar precios de piezas si aplica). **No tiene acceso a Ventas ni a la caja/turno por defecto** — no cobra directamente; deja la reparación en "Lista para entrega" y es el Cajero (o el Admin) quien cobra al momento de la entrega. Si en un negocio pequeño la misma persona hace ambas cosas, el Admin puede crear un rol personalizado que combine los permisos de Cajero y Técnico — ver "Roles personalizados" abajo.
- **Roles personalizados**: el Admin puede crear roles nuevos (ej. "Encargado de inventario", "Supervisor", o una combinación de Cajero + Técnico si una sola persona hace ambas funciones) y asignarles una combinación de permisos de una lista predefinida — no texto libre. Esa lista de permisos disponibles debe quedar cerrada y documentada en `docs/DECISIONS.md` antes de codificar, para que sea la referencia de qué combinaciones son posibles.

Modelo de datos sugerido:
- `Roles` (con flag `EsFijo` para proteger Super Admin, Cajero y Técnico contra eliminación/edición de sus permisos base).
- `Permisos` (catálogo cerrado de permisos disponibles, ej. `Ventas.Crear`, `Ventas.Cobrar`, `Ventas.Anular`, `Turno.Gestionar`, `Reparaciones.Gestionar` (crear/editar/diagnosticar), `Reparaciones.Cobrar` (marcar entregada + registrar el pago), `Clientes.Gestionar`, `Productos.Ver`, `Productos.Editar`, `Reportes.Ver`, `Usuarios.Gestionar`, etc.).
...
- Semilla (seed) automática al instalar: 1 Super Admin fijo + 1 Admin inicial (configurado por el dueño en el primer arranque) + roles predefinidos **Cajero** y **Técnico**, cada uno con sus permisos base ya descritos arriba.
- Pantalla donde el Admin (o el Super Admin) gestiona empleados: crear, editar, desactivar (no eliminar directamente, para conservar la trazabilidad de qué empleado hizo qué venta o reparación).
