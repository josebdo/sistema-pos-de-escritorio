...
Objetivo: gestión completa del catálogo de productos, organizados por categoría, cada uno con un SKU único.

Alcance mínimo:
- CRUD de categorías (nombre, descripción).
- CRUD de productos: nombre, descripción, categoría (FK), SKU (único, indexado), precio de costo, precio de venta, stock actual, **cantidad mínima** (para la alerta de stock bajo, ver fase siguiente), código de barras (campo que usará la fase de código de barras).
...
- No eliminar productos con ventas o compras asociadas — desactivar en su lugar (mismo criterio que empleados: se conserva trazabilidad).
...
Definition of Done: CRUD probado, unicidad de SKU validada por test, build y tests pasando.
...
- Al confirmar la compra:
  - El stock de cada producto comprado se incrementa en la cantidad comprada.
...
Definition of Done: probado registrando una compra con un producto que cambia de costo, verificando que el stock aumentó y el costo se actualizó correctamente; build y tests pasando.
...
2. Fase: Apertura y cierre de caja (turno) — depende de tener usuarios autenticados.
3. Fase: Productos, categorías y SKU — base del inventario; el resto de fases de inventario y ventas dependen de esta.
4. Fase: Proveedores
5. Fase: Compras a proveedor (actualiza inventario)
6. Fase: Alertas de stock mínimo
7. Fase: Gastos e ingresos del negocio
8. Fase: Código de barras — lectura
9. Fase: Código de barras — generación
10. Fase: Métodos de pago
11. Fase: Modo caja única / multi-caja — al final, porque es la decisión arquitectónica de mayor impacto y afecta a todas las fases anteriores; conviene construir y probar bien el sistema en modo caja única primero, y luego agregar la capacidad de activar multi-caja.
